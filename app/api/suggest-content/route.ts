...
function getRelevantPreApprovedAssets(type: string, issuer?: string, product?: string): string[] {
  ...
  const assetType = type === "headline" ? "headlines" : "primary-text"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const contentType = type === "headline" ? "headline" : "primaryText"
    ...
    if (!existingContent || existingContent.length === 0) {
      ...
      const fallbacks = contentType === "headline" ? fallbackSuggestions.headlines : fallbackSuggestions.primaryText
